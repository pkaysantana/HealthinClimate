...
@st.cache_data(show_spinner=False)
def get_timeline(site_key: str, day: str) -> dict:
...
worker_view = st.sidebar.radio("Worker", ["Veteran (acclimatized)", "New worker (day 0)"])
...
tl = get_timeline(site_key, day) if day != demo["focus_day"] else demo["timeline"]
rows = tl["rows"]
...
# ---- compliance ------------------------------------------------------------
st.subheader("Tamper-evident compliance log")
